...
public class Print {
    public static void print_arraylist_family(ArrayList<Family> tobedisplayed) {
        ...
        System.out.format("%5s %12s%11s %10s%4s %10s%4s %12s%11s %12s%8s %12s%11s %12s%8s %43s%30s", "|", columnNames[0], "|", columnNames[1], "|", columnNames[2], "|", columnNames[3], "|", columnNames[4], "|", columnNames[5], "|", columnNames[6], "|", columnNames[7], "|");
        System.out.println(" ");
        ...
        for (int i = 0; i < tobedisplayed.size(); i++) {
            ...
            if (obj.Divorced != null) {
                System.out.format("%5s %1s%1s %12s%2s %12s%2s %1s%1s %16s%4s %1s%1s %16s%4s %72s%1s", "|",
                        obj.ID, "|",
                ...
                System.out.format("%5s %1s%1s %12s%2s %12s%2s %1s%1s %16s%4s %1s%1s %16s%4s %72s%1s", "|",
                        obj.ID, "|",
                        obj.Married == null ? null : obj.Married.toString().substring(0, 11), "|",
                        obj.Divorced == null ? null : obj.Divorced.toString().substring(0, 11), "|",
                        obj.HusbandID, "|",
                        obj.HusbandName, "|",
                        obj.WifeID, "|",
                        obj.WifeName, "|",
    ...
    public static void print_arraylist_indivdual(ArrayList<Indivdual> tobedisplayed) {
